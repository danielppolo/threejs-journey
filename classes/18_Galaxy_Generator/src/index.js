...
import {
  BufferAttribute, BufferGeometry, MeshStandardMaterial, PointsMaterial,
  Points,
  Color,
} from 'three'
...
gui.add(parameters, 'size').min(0.001).max(0.1).step(0.001)
  .name('Particle size')
  .onFinishChange(generateGalaxy)
gui.add(parameters, 'radius').min(0.01).max(10).step(0.01)
  .name('Radius')
  .onFinishChange(generateGalaxy)
...
gui.add(parameters, 'randomness').min(0).max(2).step(0.001)
  .name('Randomness')
  .onFinishChange(generateGalaxy)
gui.add(parameters, 'randomnessPower').min(0).max(2).step(0.001)
  .name('Randomness power')
  .onFinishChange(generateGalaxy)
gui.add(parameters, 'insideColor')
  .name('Inside color')
  .onFinishChange(generateGalaxy)
gui.add(parameters, 'outsideColor')
  .name('Outside color')
  .onFinishChange(generateGalaxy)
